CREATE DATABASE IF NOT EXISTS `cralwer`;
...
CREATE TABLE IF NOT EXISTS `task_items` (
    ...
    `desc` varchar(256) DEFAULT NULL COMMENT '图片描述', 
    `url` varchar(1024) NOT NULL COMMENT '图片外链', 
    `filepath` varchar(256) DEFAULT NULL COMMENT '图片的本地保存地址', 
    `digest` varchar(32) DEFAULT NULL COMMENT '图片内容的hash签名，可用于去重',
    `status` tinyint(2) unsigned NOT NULL DEFAULT '0' COMMENT '状态: 0图片未下载, 1下载失败, 2下载成功',
